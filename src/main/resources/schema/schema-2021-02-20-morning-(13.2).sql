...
--
-- Name: calc_avg_tab; Type: TABLE; Schema: public; Owner: postgres
--
...
--
-- Name: calc_mfi_tab; Type: TABLE; Schema: public; Owner: postgres
--
...
--
-- Name: calc_rsi_tab; Type: TABLE; Schema: public; Owner: postgres
--
...
--
-- Name: date_year_back_view1; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.date_year_back_view1 AS
 SELECT (src.trade_date - 365) AS from_date,
    src.trade_date AS to_date
   FROM ( SELECT DISTINCT nse_cash_market_tab.trade_date
           FROM public.nse_cash_market_tab) src
  ORDER BY src.trade_date;


ALTER TABLE public.date_year_back_view1 OWNER TO postgres;
...
--
-- Name: sector_watch_view1; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.sector_watch_view1 AS
 SELECT date_tab.trade_date,
    date_part('week'::text, date_tab.trade_date) AS week_num1,
    date_part('week'::text, date_tab.trade_date) AS week_num2,
    sum(date_tab.nifty) AS nifty,
    sum(date_tab.bank) AS bank,
    sum(date_tab.fmcg) AS fmcg,
    sum(date_tab.pharma) AS pharma,
    sum(date_tab.auto) AS auto,
    sum(date_tab.metal) AS metal,
    sum(date_tab.it) AS it
   FROM ( SELECT index_tab.trade_date,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty 50'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS nifty,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty Bank'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS bank,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty FMCG'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS fmcg,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty Pharma'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS pharma,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty Auto'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS auto,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty Metal'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS metal,
                CASE
                    WHEN ((index_tab.symbol)::text = 'Nifty IT'::text) THEN round((round((index_tab.close / nifty_tab.nclose), 2) * (100)::numeric), 0)
                    ELSE (0)::numeric
                END AS it
           FROM ( SELECT nse_index_market_tab.trade_date AS td,
                    nse_index_market_tab.symbol AS nsymbol,
                    nse_index_market_tab.close AS nclose
                   FROM public.nse_index_market_tab
                  WHERE ((nse_index_market_tab.symbol)::text = 'Nifty 50'::text)) nifty_tab,
            public.nse_index_market_tab index_tab
          WHERE ((nifty_tab.td = index_tab.trade_date) AND ((index_tab.symbol)::text = ANY (ARRAY[('Nifty 50'::character varying)::text, ('Nifty Auto'::character varying)::text, ('Nifty Bank'::character varying)::text, ('Nifty FMCG'::character varying)::text, ('Nifty IT'::character varying)::text, ('Nifty Metal'::character varying)::text, ('Nifty Pharma'::character varying)::text])))) date_tab
  GROUP BY date_tab.trade_date
  ORDER BY date_tab.trade_date;


ALTER TABLE public.sector_watch_view1 OWNER TO postgres;
...
--
-- Name: calc_avg_tab calc_avg_unique; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: calc_mfi_tab calc_mfi_unique; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: calc_rsi_tab calc_rsi_unique; Type: CONSTRAINT; Schema: public; Owner: postgres
--
